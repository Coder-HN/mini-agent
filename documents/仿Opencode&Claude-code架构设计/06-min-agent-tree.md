...
| 阶段 | 目标 |
|------|------|
| **P0** | 一条对话跑通：FC → query → 桩数据 → 人话 → PG 落库 |
| **P1** | 边做边提示（tool_start/end、text_delta → SSE）、流式 Turn、权限两道门、历史截断、HTTP 中间件、`question` |
| **P1** | 话术 `talk_assistant` HTTP 挂 ai_crew（方案 A）；流式/权限另见 `p1-streaming-permission` |
| **P2** | `task` / 纪要·PPT 等固定流水线、长期记忆、skill/@引用；Loop 步数软收口见 `p2-loop-max-steps-align` |
| **P3** | 上下文压缩 compaction（见 `p3-context-compaction`）；MCP、后台任务、联网工具、ToolSearch 级治理 |
...
│       └── middleware.py        P1
└── apps/
    ├── min_agent/                 # 前台 FC Agent（默认）
    │   ├── agent.py · config.py · main.py · cli.py   P0
    │   ├── gateway.py                               P0 佣金桩 + Go 预留
    │   └── tools/query.py                           P0
    │       navigate.py                              P0/P1
    │       talk_assistant.py                        P1 HTTP 挂载 ai_crew（不迁 Flow）
    │       meeting_minutes.py                       P2+ 委托纪要 Workflow
    │       ppt_generate.py                          P2+ 委托 PPT Workflow
    ├── （话术本体仍在 crm-ai/ai_crew，不进本仓）
    ├── meeting_minutes/           # 会议纪要 Workflow（后续）
    └── ppt/                       # PPT Workflow（后续）
```

共享进 `packages/`；前台装配进 `apps/min_agent`；话术/纪要/PPT 等固定流水线各自成 app，经 tool 挂到前台（方案 A）。`task` 属平台能力，放 `agent_core`。
...
```text
用户 → POST /chat 或 CLI
  → Loop 第1轮：query(commission, store_name=睿德志行)
  → gateway 桩：2 条已驳回
  → Loop 第2轮：人话总结
  → PostgreSQL 有 user/assistant（及 session）
```

不做：流式、边做边提示、ask 权限、compaction、task、真实 commission API。
...
| Change | 阶段 |
|--------|------|
| `p0-chat-fc-loop` | P0（已建） |
| `p1-streaming-permission` | P1：边做边提示 + 流式 + 权限 |
| `p1-talk-assistant` | P1：话术 HTTP 挂 ai_crew（已/将归档） |
| `p2-task-subagent` | P2：task + 纪要/PPT 固定流水线挂载 |
