# talk-assistant-mount

## Purpose

话术分析经 HTTP 挂载已上线 ai_crew（方案 A）；`talk_assistant` tool 同步轮询至 `reply_done` 并回灌摘要。
...
#### 场景：禁止迁入编排引擎
- **当** 实现本挂载
- **则** min-agent 依赖中不得新增 CrewAI；execute 路径不得直接实例化 `SalesScriptFlow`
